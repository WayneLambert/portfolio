...
class TestPostForm:
    ...
    sample_form_data = helpers.get_sample_form_data()
    ...
    def test_form_title_requires_min_num_chars(self):
        form = PostForm({'title': 'an example title'})
        error_returned = form.errors['title'][0]
        assert error_returned.startswith('Ensure this value has at least 40 characters (it has ')

    def test_title_field_contains_help_text(self):
        form = PostForm(data=self.sample_form_data)
    ...
    def test_categories_field_is_required(self):
        form = PostForm({'categories': ''})
        error_returned = form.errors['categories'][0]
        assert error_returned == 'This field is required.'

    def test_categories_field_contains_help_text(self):
        form = PostForm(data=self.sample_form_data)
        help_text = form.fields['categories'].help_text
        assert help_text == 'Select more than one category by holding down Ctrl or Cmd key'

    def test_image_field_contains_help_text(self):
        form = PostForm(data=self.sample_form_data)
