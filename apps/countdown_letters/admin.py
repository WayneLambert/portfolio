...
class LettersGameAdmin(admin.ModelAdmin):
    model = LettersGame
    list_display = ("letters_chosen", "players_word", "comp_word", "winning_word", "entry_date")
    ordering = ("-entry_date",)
    ...
    fieldsets = (
        ("Selection", {"fields": ["letters_chosen"]}),
        ("Game", {"fields": (("players_word", "eligible_answer", "comp_word", "winning_word"))}),
        (
            "Results",
            {"fields": (("player_word_len", "player_score"), ("comp_word_len", "comp_score"))},
        ),
        (
            "Definition",
            {
